function drawPie(postal) {
	...
	let raceData = {};
	let stateData = policeData.filter(d => d["Incident.Location.State"] == postal)
	stateData.forEach(e => {
		if(e["Person.Gender"] == "Male")
			genderData["Male"]++;
		else
			genderData["Female"]++;
		if(raceData[e["Person.Race"]] == null)
			raceData[e["Person.Race"]] = 1;
		else
			raceData[e["Person.Race"]]++
	});
	let colorScaleGender = d3.scaleOrdinal()
		.domain(genderData)
		.range(d3.schemeSet1);
	let colorScaleRace = d3.scaleOrdinal()
		.domain(raceData)
		.range(d3.schemeSet2);
	let pie = d3.pie()
		.value(d => d.value)
	...
	let genderSvg = pieSvg.append("g")
		.attr("width", pieWidth)
		.attr("height", pieHeight)
		.attr('transform', `translate(${pieWidth/2+margin.left},${height/2})`)
	...
	genderSvg.selectAll('genderSlices')
		.data(pieGenderData)
		.enter()
		.append('path')
			.attr('d', arcGen)
